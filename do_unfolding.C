...
int main(int argc, char* argv[])
{
  ...
  else if(strcmp(argv[2], "muon_elecJMS") == 0) channel = "muon_elecJMS";
  else if(strcmp(argv[2], "elec_muonJMS") == 0) channel = "elec_muonJMS";
  ...
  */
  ...
  vector<TString> jms_name = {"jmsupup", "jmsdowndown"};
  ...
  */
  ...
  vector<TString> cr = {"CR_gluonmove", "CR_qcdbased"};
  ...
  vector< vector<TString> > model_name = {fsr, mass, cr, isr, hdamp, tune, scale};
  vector<TString> model_rel_name = {"FSR", "mass", "CR", "ISR", "hdamp", "UEtune", "scale"};
  vector<bool> do_average =        {true, true, false, true, true, true, true};

  if(year == "FullRun2"){
    model_name =        { mass,    cr,   isr,   hdamp,     tune,   scale};
    model_rel_name =    {"mass", "CR", "ISR", "hdamp", "UEtune", "scale"};
    do_average =        {true,  false,  true,    true,     true,    true};
  }
  ...
  */
  ...
  vector<TString> fsr_theo = {"SHOWER_fsrdown_2", "SHOWER_fsrup_2"};

  vector< vector<TString> > theo_name = {scale, isr, fsr_theo, hdamp, tune, cr};
  vector<TString> theo_rel_name = {"scale", "ISR", "FSR", "hdamp", "UEtune", "CR"};
  // vector< vector<TString> > theo_name = {scale, fsr, isr};
  // vector<TString> theo_rel_name = {"scale", "FSR", "ISR"};
  ...
  */
  ...
  double mtopuncert_min = 171.5;
  double mtopuncert_max = 173.5;
  ...
  */
  ...
  for(unsigned int i=0; i<sys_name.size(); i++){
    for(unsigned int j=0; j<sys_name[i].size(); j++){
      CovSys[i][j]->Write("COV_"+sys_name[i][j]);
  ...
  for(unsigned int i=0; i<SYS_rel.size(); i++) SYS_rel[i]->Write("DELTA_"+sys_rel_name[i]);
  ...
  if(do_model){
    ...
    for(unsigned int i=0; i<MODEL_rel.size(); i++) MODEL_rel[i]->Write("DELTA_"+model_rel_name[i]);
...
vector<TF1*> FitBinHistsForFSR(vector<TGraphErrors*> hists){
  vector<TF1*> fits;
  for(auto h: hists){
    TF1*f1 = new TF1("f1","[0] + [1]*log(x) + [2]*x");
    h->Fit("f1");
    TF1* fit = h->GetFunction("f1");
    fits.push_back(fit);
  }
...
TH1* GetDeltaFromBinHists(vector<TF1*> fits, TH1* dummy, double xmin, double xmax){
  ...
  for(unsigned int i=0; i<fits.size(); i++){
    ...
    double sign = 1.0;
    if( fits[i]->GetParameter(1) < 0 ) sign = -1.0;
    delta->SetBinContent(i+1,  d*sign);
  }
  return delta;
}
